...
@app.timer_trigger(schedule="0 */5 * * * *", arg_name="_")
async def timer_trigger(_: func.TimerRequest, context: func.Context) -> None:
...
async def GetRecord(_: func.HttpRequest, context: func.Context) -> func.HttpResponse:
    with tracer.start_as_current_span("Execute command", context=transform_context(context)):
        await depencencies_builder.get_command().execute()
