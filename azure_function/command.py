...
class Command:
    ...
    async def execute(self) -> None:
        async with aiohttp.ClientSession() as session:
            with self.__tracer.start_as_current_span(name="Get Google Page", context=get_context(), kind=SpanKind.CLIENT) as span:
                span.set_attribute("db.system", "MyAPI")
            ...
            with self.__tracer.start_as_current_span(name="Get Microsoft Page", context=get_context(), kind=SpanKind.CLIENT) as span:
                span.set_attribute("db.system", "MyAPI")
